...
def _store_uploaded_measurement(name: str, content: str) -> str:
    ...
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=suffix, prefix="evifluor-load-", delete=False) as handle:
...
def create_app(sim_host: str = "127.0.0.1", sim_port: int = 5000) -> FastAPI:
    app = FastAPI(
        title="eviFluor Simulator Demo",
        version="1.0",
        description="Web UI for selected eviFluor simulator control commands.",
    )
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>eviFluor Simulator Demo</title>
  <style>
    :root {
      --bg: #edf1f4;
      --panel: #ffffff;
    ...
    * { box-sizing: border-box; }
    body {
      ...
      color: var(--ink);
      background:
        radial-gradient(circle at top left, rgba(207, 63, 63, 0.18) 0, transparent 30%),
        linear-gradient(135deg, #f5f7f9, var(--bg) 48%, #e8edf1);
      display: grid;
      place-items: center;
      padding: 24px;
    }
...
<body>
  <main>
    <h1>eviFluor Simulator</h1>
    <p>Control UI for selected simulator commands on the running <code>eviFluor</code> instance.</p>
    <section class="status">
      <div>Current cuvette-holder state</div>
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="python -m hse.simulator.evifluor.web",
        description="Start a minimal web UI for the eviFluor simulator.",
    )
    parser.add_argument("--host", default="127.0.0.1", help="Bind host for the web UI")
    parser.add_argument("--port", type=int, default=8010, help="Bind port for the web UI")
