...
class Channel:
    ...
    def __init__(self, dark = 0.0, value = 0.0, led_power = 0.0):
        ...
        Args:
            dark: The dark value in mV. Defaults to 0.0.
            value: The measured value in mV. Defaults to 0.0.
            led_power: The LED power without unit, typically in the range 0..255.
                Defaults to 0.0.
        """
        self.dark      = dark
        self.value     = value
    ...
    def from_json(node):
        ...
        Args:
            node: JSON object containing ``dark``, ``value``, and ``led_power``.

        Returns:
            A new :class:`Channel` instance populated from ``node``.
        """
        return Channel(
            node[DictKeys.DARK],
            node[DictKeys.VALUE],
