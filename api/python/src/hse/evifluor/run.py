...
class Run:
    ...
    def __init__(self, nr_of_std_low, nr_of_std_high, concentration, path = None, filename = None, device = None, no_air = False, kit = DefaultKit(), settling_time = None):
        ...
        Args:
            nr_of_std_low: Number of low-standard replicates to collect before
                factor calculation.
            nr_of_std_high: Number of high-standard replicates to collect before
                factor calculation.
            concentration: Concentration assigned to the high standard.
            path: Optional directory for the output JSON file.
            filename: Optional measurement filename. If omitted, a filename is
                generated automatically.
            device: Optional device serial number, ``"SIMULATION"``, or an
                already opened device instance.
            no_air: If ``True``, the run skips air measurements and uses the
                sample-only algorithm.
            kit: Optional kit overriding the default fit model and measurement
                settings.
    ...
    def load_state(filename = None):
        ...
        Args:
            filename: Optional explicit state filename. If omitted, the default
                state filename is resolved.

        Returns:
            A reconstructed :class:`Run` instance.
        """
    ...
    def save_state(self, filename = None):
        ...
        Args:
            filename: Optional explicit state filename overriding the default
                naming.
        """
        logger.debug(
    ...
    def skip_air(self):
        """Skips the next air measurement and reuses the previous air value.

        Raises:
            Exception: If the current state does not allow skipping the air
                measurement or no reusable air value is available.
        """
