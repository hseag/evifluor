...
#define RUN_INIT_K1_OPTION                "--k1="
#define RUN_INIT_K2_OPTION                "--k2="
#define RUN_INIT_K3_OPTION                "--k3="
#define RUN_INIT_LOOKUP_TABLE_OPTION      "--lookup_table="
#define RUN_INIT_LOOKUP_TABLE_OPTION_ALT  "--lookup-table="
            ...
            if(strcmp(argvCmdSave[0], "init") == 0)
            {
                ...
                if(argcCmdSave >= 4)
                {
                    for(int argIndex = 4; argIndex < argcCmdSave; argIndex++)
                    {
                        ...
                        else if(strncmp(argvCmdSave[argIndex], RUN_INIT_K1_OPTION, strlen(RUN_INIT_K1_OPTION)) == 0)
                        {
                            options.hasK1Override = true;
                            options.k1Override = strtod(argvCmdSave[argIndex] + strlen(RUN_INIT_K1_OPTION), NULL);
                        }
                        else if(strncmp(argvCmdSave[argIndex], RUN_INIT_K2_OPTION, strlen(RUN_INIT_K2_OPTION)) == 0)
                        {
                            options.hasK2Override = true;
                            options.k2Override = strtod(argvCmdSave[argIndex] + strlen(RUN_INIT_K2_OPTION), NULL);
                        }
                        else if(strncmp(argvCmdSave[argIndex], RUN_INIT_K3_OPTION, strlen(RUN_INIT_K3_OPTION)) == 0)
                        {
                            options.hasK3Override = true;
                            options.k3Override = strtod(argvCmdSave[argIndex] + strlen(RUN_INIT_K3_OPTION), NULL);
                        }
                        else if(strncmp(argvCmdSave[argIndex], RUN_INIT_LOOKUP_TABLE_OPTION, strlen(RUN_INIT_LOOKUP_TABLE_OPTION)) == 0)
                        {
                            free(options.lookupTablePath);
                            options.lookupTablePath = strdup(argvCmdSave[argIndex] + strlen(RUN_INIT_LOOKUP_TABLE_OPTION));
                        }
                        else if(strncmp(argvCmdSave[argIndex], RUN_INIT_LOOKUP_TABLE_OPTION_ALT, strlen(RUN_INIT_LOOKUP_TABLE_OPTION_ALT)) == 0)
                        {
                            free(options.lookupTablePath);
                            options.lookupTablePath = strdup(argvCmdSave[argIndex] + strlen(RUN_INIT_LOOKUP_TABLE_OPTION_ALT));
                        }
                        else if(strncmp(argvCmdSave[argIndex], RUN_INIT_SETTLING_TIME_OPTION, strlen(RUN_INIT_SETTLING_TIME_OPTION)) == 0)
                        {
                            ...
                            options.settlingTimeOverride = strtod(argvCmdSave[argIndex] + strlen(RUN_INIT_SETTLING_TIME_OPTION), NULL);
                        }
                        else
                        {
                    ...
                    if(options.hasK1Override)
                    {
                        options.kit.k1 = options.k1Override;
                    }
                    if(options.hasK2Override)
                    {
                        options.kit.k2 = options.k2Override;
                    }
                    if(options.hasK3Override)
                    {
                        options.kit.k3 = options.k3Override;
                    }
                    if(options.lookupTablePath != NULL)
                    {
                        if(!kit_lookupTableLoad(options.lookupTablePath, &options.kit))
                        {
                            ret = printError(ERROR_EVI_UNKOWN_COMMAND_LINE_ARGUMENT, "Invalid lookup table: %s", options.lookupTablePath);
                            goto cleanup_context;
                        }
                    }
