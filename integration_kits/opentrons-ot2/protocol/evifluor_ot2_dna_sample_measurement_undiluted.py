...
metadata = {
    "apiLevel": "2.27",
    "protocolName": "eviFluor undiluted sample measurement",
    "author": "HSE AG",
    "version": "0.1-draft",
}
...
class SparkleVerificationProtocol:
    def __init__(self, protocol):
        ...
        self.standard_high_tube.set_volume(V_DEAD_TUBE + V_SAMPLE_ASSAY)
        self.standard_low_tube.set_volume(V_DEAD_TUBE + V_SAMPLE_ASSAY)
        self.standard_high_assay_tube.set_volume(V_WS_ASSAY + V_SAMPLE_ASSAY)
        self.standard_low_assay_tube.set_volume(V_WS_ASSAY + V_SAMPLE_ASSAY)
    ...
    def report_tube_setup(self):
        self.protocol.comment(
            "Tube setup [uL]: C1 working solution {:.1f}, A2 standard high {:.1f}, "
            "B2 standard low {:.1f}; A3/B3 receive 40.0 uL standard assays. "
    ...
    def prefill_mix(self):
        self.protocol.comment("1. Pre-fill MIX plate and standard assay tubes")
    ...
    def prepare_assays_and_park_tips(self):
        self.protocol.comment("2. Prepare fluorescence assay and park sample-specific tips")
        for src, dest, dest_container, parking in zip(
            self.sample_wells,
            self.mix_wells,
            self.mix_well_liquids,
            ...
            self.p20.pick_up_tip()
            self.p20.aspirate(
                V_SAMPLE_ASSAY, src.bottom(SAMPLE_ASPIRATION_HEIGHT)
            )
            self.dispense_with_tracking(
                V_SAMPLE_ASSAY,
                dest,
    ...
    def prepare_standards_and_park_tips(self):
        self.protocol.comment("3. Prepare standard assays and park standard-specific tips")
        standard_setups = (
            (
                self.standard_high,
        ...
        for (source, source_tube, dest, dest_tube), parking in zip(
            ...
            self.p20.pick_up_tip()
            self.aspirate_with_tracking(
                V_SAMPLE_ASSAY, source, source_container=source_tube
            )
            self.p20.dispense(V_SAMPLE_ASSAY, dest.bottom(1.0))
            self.mix_well(
                dest,
                cycles=MIX_CYCLES_ASSAY,
                offset_x=MIX_OFFSET_X_TUBE,
    ...
    def incubate(self):
        self.protocol.comment("4. Incubate")
    ...
    def measure_standards(self):
        self.protocol.comment("5. Measure standards")
    ...
    def measure_samples(self):
        self.protocol.comment("6. Measure samples")
        concentrations = []
        for index, (source_well, mix_well, mix_container, parking) in enumerate(
            ...
            result = self.measure_loaded_cuvette(
                "Sample@{}".format(source_well.well_name)
            )
            self.p20.drop_tip()
    ...
    def calculate_sample_concentrations(self, measured):
        self.protocol.comment("7. Calculate sample concentrations")
        return measured

    def report_results(self, concentrations):
        self.protocol.comment("8. Report sample concentrations")
                ...
                "samples": [
                    {
                        "sample_index": index,
                        "well": well.well_name,
                        "sample_concentration_ng_ul": concentration,
                    }
                    for index, (well, concentration) in enumerate(
                        zip(self.sample_wells, concentrations), start=1
                    )
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p.report_tube_setup()
    p.prefill_mix()
    p.prepare_assays_and_park_tips()
    p.prepare_standards_and_park_tips()
    p.incubate()
    p.measure_standards()
    measured = p.measure_samples()
    concentrations = p.calculate_sample_concentrations(measured)
