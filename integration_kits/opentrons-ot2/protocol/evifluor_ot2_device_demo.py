...
metadata = {'apiLevel': '2.24',
            'protocolName': 'eviFluor OT-2 Device Demo',
            'author': 'DaZw',
            'version': '1.0.0'}
...
class Instrument:
    ...
    def __init__(self, protocol):
        ...
        self.total_positions = (
            ...
            + self.protocol.params.number_of_samples
        )

        self.workdeck   = { 'sample_plate'    : protocol.load_labware('corning_96_wellplate_360ul_flat',    '5'),
                            'evifluor'        : protocol.load_labware('hse_evifluor_pilot_left_20ul_tip_v2','7'),
                            'tiprack_20'      : protocol.load_labware('opentrons_96_filtertiprack_20ul',    '6'),
                          }
        
        self.workdeck   = dotdict(self.workdeck)
        well_slice = slice(self.mtp_offset, self.mtp_offset + self.total_positions)
        self.sample_wells = self.workdeck.sample_plate.wells()[well_slice]
    ...
    def sample_aspirate(self, volume, position, height=1):
        well = self.sample_wells[position]
    ...
    def comment(self, position):
        well_name = self.sample_wells[position].well_name
        ...
        return "{}@{}".format(sample_type, well_name)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    total_positions = (
        protocol.params.nr_of_std_high
        + protocol.params.nr_of_std_low
        + protocol.params.number_of_samples
    )
    mtp_start_well = str(protocol.params.mtp_start_well)
    assert total_positions <= 96, "The sum of <standard high> + <standard low> + <Number of samples> must be less or equal 96."
    assert mtp_start_well in MTP_START_WELL_TO_OFFSET, "The MTP start well must be between A1 and H12."
    assert MTP_START_WELL_TO_OFFSET[mtp_start_well] + total_positions <= 96, "The selected MTP well range must fit on the sample plate."
    ...
    if not protocol.is_simulating():
        ...
        i.run.storage.add_dict("parameters", { 'number_of_samples'  : protocol.params.number_of_samples, 
                                               ...
                                               'mtp_start_well'     : i.mtp_start_well,
...
def add_parameters(parameters):
    ...
    parameters.add_int(
        ...
        description="Number of high-standard positions at the start of the selected source-well range.",
        default = 1,
        minimum = 1,
        maximum = 4
    )
    ...
    parameters.add_int(
        ...
        description="Number of low-standard positions following the high-standard positions.",
        default = 1,
        minimum = 1,
        maximum = 4
    )
    ...
    parameters.add_str(
        ...
        description="First sample-plate well. Positions follow the order A1, B1, ..., H12.",
