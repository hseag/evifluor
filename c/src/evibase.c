...
Error_t eviCommandComm(Evi_t *self, HANDLE hComm, const char * command, EvieResponse_t *response)
{
    Error_t ret = ERROR_EVI_OK;
		...
		if (hComm != INVALID_HANDLE_VALUE)
		{
            ...
            if (eviPortRead(hComm, response->response, EVI_MAX_LINE_LENGTH, self->verbose) > 0)
			{
                ...
                for (int i = 0; (i < EVI_MAX_LINE_LENGTH) && (d[i] != 0) && (response->argc < EVI_MAX_ARGS); i++)
				{
					isSpace = isspace(d[i]);
